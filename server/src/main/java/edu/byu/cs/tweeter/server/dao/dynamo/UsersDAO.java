...
public class UsersDAO extends DynamoDAO<User> {
    ...
    public User get(String user_alias, String sort) throws DataAccessException {
        ...
        if (user == null) {
            ...
            return new User(user.getFirst_name(), user.getLast_name(), user.getUser_alias(), user.getImage_url(), user.getFollower_count(), user.getFollowing_count(), user.getPassword(), user.getSalt());
        }
    }
